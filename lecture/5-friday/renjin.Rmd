...
<<<<<<< HEAD
With a little bit more thinking, we can come up with a second approach, which uses vectors that are half the length of `m`.
=======
With a little bit more thinking, we can come up with a second vectorized approach, which avoids calculating `(-1)^n`. The approach also uses vectors that are 1/2 the full length.
>>>>>>> 23aa989248387139f2cfc97039ebdbd6141c28f9
