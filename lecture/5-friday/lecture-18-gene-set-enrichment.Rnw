...
\begin{frame}{Approach 1: hypergeometric tests}
  \begin{columns}
    ...
    \item Fisher hypergeometric test. \Biocpkg{GOstats};
      \Biocpkg{edgeR}\Rfunction{::goana()} \ldots
    \end{enumerate}
    \begin{itemize}
...
\begin{frame}{Approach 2: enrichment score}
  \begin{columns}
    ...
    \end{enumerate}
    \column{.5\textwidth}
    \includegraphics[width=\textwidth]{subramanian-F1-part.jpg}
...
\begin{frame}{Approach 3: category $t$-test}
  \begin{columns}
    ...
    \end{enumerate}
    \column{.5\textwidth}
    \includegraphics[width=\textwidth]{Category-ribosome.png}
...
\begin{frame}{Approach 5: issues with sequence data?}
  \begin{columns}
    ...
    \end{itemize}
    \column{.5\textwidth}
    \includegraphics[width=\textwidth]{young-et-al-gb-2010-11-2-r14-2-cropped.png}
...
\begin{frame}[fragile, allowframebreaks]{Example -- \Biocpkg{DESeq2} lab}
  Example: hypergeometric test using \Biocpkg{edgeR}\Rcode{::goana()}
  \begin{itemize}
...
<<trim>>=
xtabs(~ is.na(padj) + is.na(egid), res)
