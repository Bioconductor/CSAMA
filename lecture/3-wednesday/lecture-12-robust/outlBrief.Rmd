...
- Much work in robustness addresses distributional form and estimation of
location and scale
- "Natural robustness" has been mentioned: modest departures from normality don't have substantial effect on validity of inference with $t$-like statistics
- Departures from **independence** could invalidate seemingly careful inferences
    - batch-related variation
    - autocorrelation
    - impact: the actual sample size is smaller than advertised/used in variance estimation
- Diagnostics/remedies for non-independence not ready to hand, requires manual work
...
values if 50% or more of the data are arbitrarily moved
    - intermediate estimators: trimmed means, winsorized means

## Scale estimation

- define the residuals $r_i = x_i -\hat{L}$, where $\hat{L}$ is
a location estimator
    - SD: $\hat{L} = \bar{x}$, $s = \sqrt{\sum_i r_i^2/(n-1)}$
    - MAD: $\hat{L} = \mbox{med}~{x}$, $s = 1.4826\cdot\mbox{med}~{|r_i|}$
    - length of the shorth (no location estimation): $u-l$ where
$u$ is the largest value and $l$ the smallest value of the shortest
half-sample
    - IQR (no location estimation): `quantile(x, .75)-quantile(x, .25)`
- scale estimation is important for 'normalization'
    - scale invariance: $x$ and $x/c$ obey the same laws
    - informally: can transform all samples to possess a common scale without compromising interpretation regarding differential expression, pathway role, etc.
    - why make such a transformation?  
         - "rough comparability", "effect size interpretation"
