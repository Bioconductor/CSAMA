...
Estimation-Maximization or Bayesian techniques to estimate the
abundances and counts. 
Following quantification, we will use the `r Biocpkg("tximport")` [@Soneson2015Differential]
...
GENCODE has already released a newer version, but for computational
...
*Salmon* is up to version 0.10.1 and it's always best to use the
...
We load a *data.frame* that connects transcripts to genes for GENCODE v27,
`tx2gene`. We will show how to construct such a table in a following
lab. One can also find example code in the `r Biocpkg("tximport")` package
vignette. The information in `tx2gene` needs to directly match the
reference transcriptome used by *Salmon*. We can ensure this by
downloading the *FASTA* and *GTF* from the same version from GENCODE.
...
block) contains the matrix of counts, the `rowRanges` or `rowData`
(blue block) contains information about the genomic ranges and the
`colData` (green block) contains information about the samples. The
highlighted line in each block represents the first row (note that the
first row of `colData` lines up with the first column of the `assay`).
