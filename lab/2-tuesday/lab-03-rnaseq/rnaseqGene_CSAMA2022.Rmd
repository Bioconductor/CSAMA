...
The quantification directories are in the `quants` folder. 

```{r}
list.files(file.path(dir, "quants"))
```

The identifiers used here are the *SRR* identifiers from the [Sequence Read Archive](https://www.ncbi.nlm.nih.gov/sra). 
...
They also calculate average transcript lengths for each gene and each sample, which can be used as offsets to improve the differential expression analysis by accounting for differential isoform usage across samples [@Soneson2015Differential].
...
The assays are created by directly importing the values from the `quant.sf` files and combining this information across the eight samples:
...
Each of the packages we will use for differential expression has a specific class of object used to store the summarization of the RNA-seq experiment and the intermediate quantities that are calculated during the statistical analysis of the data.
...
The first plot below visualizes the fitted values from the model, in terms of the estimated model coefficients. 
